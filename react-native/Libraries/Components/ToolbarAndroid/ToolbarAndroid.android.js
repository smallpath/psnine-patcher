...
var ToolbarAndroid = React.createClass({
  ...
  render: function() {
    ...
    nativeProps.style = {
      ...flatten(nativeProps.style),
      elevation: 0
    }
    const toolbar = <NativeToolbar onSelect={this._onSelect} {...nativeProps} />;
    if (this.props.origin) return toolbar
    const { deepColor } = global.modeInfo
    return (
      <View style={{
        backgroundColor: deepColor,
        height: 56 + (StatusBar.currentHeight || 24),
        elevation: 4,
        zIndex: -1
      }}>
        <View style={{height: (StatusBar.currentHeight || 24), elevation: 0}}></View>
        {toolbar}
      </View>
    )
  },
